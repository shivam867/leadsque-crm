...
const CATEGORY_CONFIG = {
  price:      { label: "Price",      color: "#DC2626", bg: "#FEF2F2", border: "#FECACA", icon: DollarSign },
  timing:     { label: "Timing",     color: "#D97706", bg: "#FFFBEB", border: "#FDE68A", icon: Timer },
  competitor: { label: "Competitor", color: "#1a56db", bg: "#EFF6FF", border: "#BFDBFE", icon: Swords },
  contact:    { label: "No Contact", color: "#374151", bg: "#F8FAFC", border: "#CBD5E1", icon: PhoneOff },
  product:    { label: "Product",    color: "#7C3AED", bg: "#F5F3FF", border: "#DDD6FE", icon: Package },
  other:      { label: "Other",      color: "#059669", bg: "#ECFDF5", border: "#A7F3D0", icon: Circle },
} as const;

type CategoryKey = keyof typeof CATEGORY_CONFIG;

const TREND_CONFIG = {
  up:   { label: "Worsening", color: "#DC2626", icon: TrendingUp },
  down: { label: "Improving", color: "#059669", icon: TrendingDown },
  flat: { label: "Stable",    color: "#D97706", icon: Minus },
};
  ...
  return (
    <div style={{ padding: "28px 32px", maxWidth: 980, background: "var(--bg)", minHeight: "100%" }}>
      ...
      <div className="animate-fade-up" style={{ marginBottom: 24 }}>
        <p style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.07em", color: "#6B7280", marginBottom: 4 }}>Lead Intelligence · Director View</p>
        <h1 style={{ fontSize: 24, fontWeight: 800, color: "#111827", letterSpacing: "-0.02em", margin: "0 0 4px" }}>Lost Deal Analysis</h1>
        <p style={{ fontSize: 13, color: "#374151", margin: 0 }}>{totalLost} leads lost this month · May 2025 · Cross-team breakdown</p>
      </div>
      ...
      <div className="animate-fade-up" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginBottom: 20, animationDelay: "40ms" }}>
        {[
          { label: "Total Lost",   value: totalLost,  color: "#DC2626", bg: "#FEF2F2" },
          { label: "Price Losses", value: lostReasons.filter(r => r.category === "price").reduce((s, r) => s + r.count, 0), color: "#DC2626", bg: "#FEF2F2" },
          { label: "Competitors",  value: lostReasons.filter(r => r.category === "competitor").reduce((s, r) => s + r.count, 0), color: "#1a56db", bg: "#EFF6FF" },
          { label: "Worsening",    value: `${lostReasons.filter(r => r.trend === "up").length} reasons`, color: "#D97706", bg: "#FFFBEB" },
        ].map(s => (
          <div key={s.label} className="card" style={{ padding: "14px 16px", display: "flex", alignItems: "center", gap: 12 }}>
            <div>
              ...
              <p style={{ fontSize: 10, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em", color: "#6B7280", margin: "3px 0 0" }}>{s.label}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 20 }}>
        <div className="animate-fade-up card" style={{ padding: 18, animationDelay: "80ms" }}>
          <h2 style={{ fontSize: 14, fontWeight: 700, color: "#111827", margin: "0 0 14px" }}>Losses by Category</h2>
          <ResponsiveContainer width="100%" height={160}>
            <BarChart data={categoryTotals} margin={{ top: 0, right: 0, left: -20, bottom: 0 }} barSize={28}>
              ...
              <Bar dataKey="count" name="Lost" radius={[4, 4, 0, 0]}>
                {categoryTotals.map(c => <Cell key={c.cat} fill={c.color} fillOpacity={0.85} />)}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="animate-fade-up card" style={{ padding: 18, animationDelay: "120ms" }}>
          <h2 style={{ fontSize: 14, fontWeight: 700, color: "#111827", margin: "0 0 14px" }}>Losses per Rep</h2>
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {repLostCounts.map(rep => (
              <div key={rep.id} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <span style={{ fontSize: 12, color: "#111827", flex: 1, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                  {rep.name.split(" ")[0]}
                </span>
                <div style={{ width: 80, height: 5, background: "var(--border)", borderRadius: 99, overflow: "hidden" }}>
                  <div style={{ height: "100%", width: `${repLostCounts[0].totalLost > 0 ? (rep.totalLost / repLostCounts[0].totalLost) * 100 : 0}%`, background: rep.totalLost > 20 ? "#DC2626" : rep.totalLost > 10 ? "#D97706" : "#9CA3AF", borderRadius: 99 }} />
                </div>
                <span style={{ fontSize: 12, fontWeight: 700, minWidth: 24, textAlign: "right", color: rep.totalLost > 20 ? "#DC2626" : rep.totalLost > 10 ? "#D97706" : "#6B7280" }}>
                  {rep.totalLost}
                </span>
              </div>
            ))}
      ...
      <div className="animate-fade-up" style={{ display: "flex", gap: 6, marginBottom: 14, flexWrap: "wrap", animationDelay: "120ms" }}>
        <button onClick={() => setCatFilter("all")}
          style={{ padding: "5px 12px", fontSize: 12, fontWeight: 600, borderRadius: 8, cursor: "pointer", transition: "all .15s", background: catFilter === "all" ? "#18181B" : "var(--surface)", color: catFilter === "all" ? "#fff" : "#374151", border: `1.5px solid ${catFilter === "all" ? "#18181B" : "var(--border)"}` }}>
          All · {totalLost}
        </button>
        {(Object.entries(CATEGORY_CONFIG) as [CategoryKey, typeof CATEGORY_CONFIG[CategoryKey]][]).map(([cat, cfg]) => {
          ...
          return (
            <button key={cat} onClick={() => setCatFilter(cat)}
              style={{ display: "flex", alignItems: "center", gap: 5, padding: "5px 12px", fontSize: 12, fontWeight: 600, borderRadius: 8, cursor: "pointer", transition: "all .15s", background: isActive ? cfg.bg : "var(--surface)", color: isActive ? cfg.color : "#374151", border: `1.5px solid ${isActive ? cfg.border : "var(--border)"}` }}>
          ...
          return (
            <div key={reason.id} className="animate-fade-up card" style={{ overflow: "hidden", animationDelay: `${Math.min(i, 6) * 30}ms` }}>
              <div style={{ padding: "14px 16px", cursor: "pointer", display: "flex", alignItems: "flex-start", gap: 12 }} onClick={() => setExpanded(isExpanded ? null : reason.id)}>
                <div style={{ width: 24, height: 24, borderRadius: 7, flexShrink: 0, background: i < 3 ? cfg.bg : "var(--surface-2)", border: `1px solid ${i < 3 ? cfg.border : "var(--border)"}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 800, color: i < 3 ? cfg.color : "#6B7280" }}>
                  {i + 1}
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <p style={{ fontSize: 13, fontWeight: 600, color: "#111827", margin: "0 0 8px", lineHeight: 1.4 }}>{reason.reason}</p>
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    ...
                    <span style={{ fontSize: 10, color: "#6B7280" }}>({pct}% of losses)</span>
                  </div>
                </div>
                {isExpanded ? <ChevronUp size={14} color="#6B7280" style={{ flexShrink: 0, marginTop: 4 }} /> : <ChevronDown size={14} color="#6B7280" style={{ flexShrink: 0, marginTop: 4 }} />}
              </div>
              ...
              {isExpanded && (
                <div className="animate-fade-in" style={{ padding: "0 16px 16px", borderTop: "1px solid var(--border)", background: "var(--surface-2)" }}>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, paddingTop: 14 }}>
                    <div>
                      <p style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.07em", color: "#6B7280", marginBottom: 8 }}>By rep</p>
                      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                        {reason.repBreakdown.map(rb => (
                          <div key={rb.repName} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                            <div style={{ width: 18, height: 18, borderRadius: 5, background: "#EFF6FF", color: "#1a56db", fontSize: 7, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                              {rb.avatar}
                            </div>
                            <span style={{ fontSize: 11, color: "#111827", flex: 1 }}>{rb.repName.split(" ")[0]}</span>
                    ...
                    <div>
                      <p style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.07em", color: "#6B7280", marginBottom: 8 }}>Field notes</p>
                      <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                        {reason.examples.map((ex, j) => (
                          <div key={j} style={{ display: "flex", gap: 7, alignItems: "flex-start" }}>
                            <span style={{ color: cfg.color, flexShrink: 0, fontSize: 12, marginTop: 1 }}>›</span>
                            <span style={{ fontSize: 11, color: "#374151", lineHeight: 1.5 }}>{ex}</span>
                          </div>
                        ))}
                      </div>
                    </div>
      ...
      {/* Bottom insight */}
      <div className="animate-fade-up" style={{ marginTop: 16, padding: "14px 18px", borderRadius: 12, background: "#FEF2F2", border: "1px solid #FECACA", display: "flex", gap: 10, animationDelay: "200ms" }}>
        <Lightbulb size={18} color="#DC2626" style={{ flexShrink: 0, marginTop: 2 }} />
        <div>
          <p style={{ fontSize: 12, fontWeight: 700, color: "#991B1B", margin: "0 0 3px" }}>Director Recommendation</p>
