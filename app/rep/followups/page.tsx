  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "flex-start", gap: 10, marginBottom: 10 }}>
        <div style={{ width: 34, height: 34, borderRadius: 8, background: "var(--surface-2)", border: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 700, flexShrink: 0, color: "var(--text-secondary)" }}>
          {lead.name.split(" ").map(n => n[0]).join("").slice(0, 2)}
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <p style={{ fontSize: 14, fontWeight: 700, color: "var(--text-primary)", margin: "0 0 3px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{lead.name}</p>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <span style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 11, fontWeight: 700, color: cfg.color, background: cfg.color === "var(--text-primary)" ? "var(--surface-2)" : cfg.color + "15", padding: "2px 7px", borderRadius: 4, fontFamily: "monospace" }}>
      ...
      {lead.followUps?.[0]?.remarks && (
        <p style={{ fontSize: 12, color: "var(--text-secondary)", margin: "8px 0 0", fontStyle: "italic", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
          "{lead.followUps[0].remarks}"
        </p>
      )}
    </div>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 7, paddingBottom: 12, borderBottom: `2px solid ${items.length > 0 ? cfg.color : "var(--border)"}`, marginBottom: 12, flexShrink: 0 }}>
        ...
        <span style={{ fontSize: 14, fontWeight: 700, color: "var(--text-primary)" }}>{cfg.title}</span>
        <span style={{ fontSize: 11, fontWeight: 700, padding: "2px 8px", borderRadius: 99, background: "var(--surface-3)", color: "var(--text-secondary)" }}>
          {items.length}
        </span>
        <span style={{ fontSize: 12, color: "var(--text-secondary)" }}>— {cfg.sub}</span>
      </div>
      ...
      <div style={{ flex: 1, overflowY: "auto", display: "flex", flexDirection: "column", gap: 7 }}>
        {items.length === 0 ? (
          <div style={{ padding: "40px 0", textAlign: "center" }}>
            ...
            <p style={{ fontSize: 13, color: "var(--text-secondary)", margin: 0 }}>Nothing here</p>
          </div>
        ) : (
          items.map(lead => (
        ...
        {[
          ...
          <div key={s.label} style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 14px", background: "var(--surface-2)", border: "1px solid var(--border)", borderRadius: 8 }}>
            ...
            <div>
              ...
              <p style={{ fontSize: 12, color: "var(--text-secondary)", margin: "1px 0 0" }}>{s.label}</p>
            </div>
          </div>
        ))}
      </div>
        ...
        <div style={{ flex: 1, overflowX: "auto", overflowY: "hidden", display: "flex", paddingRight: selected && view === "list" ? 360 : 0, transition: "padding-right 0.2s ease", boxSizing: "border-box" }}>
          <div style={{ display: "flex", minWidth: "max-content", width: "100%", height: "100%" }}>
            {(["overdue", "dueToday", "upcoming"] as const).map((type, idx) => {
              ...
              return (
                <div key={type} className="animate-fade-up" style={{ width: 380, minWidth: 320, flex: "1 0 320px", padding: "20px", borderRight: idx < 2 ? "1px solid var(--border)" : "none", height: "100%", animationDelay: `${idx * 40 + 80}ms` }}>
