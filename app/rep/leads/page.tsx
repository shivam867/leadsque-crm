...
export default function RepLeads() {
  ...
  const handleSelectLead   = (lead: Lead) => { setSelected(selected?.id === lead.id ? null : lead); setView("list"); };
  ...
  return (
    <div style={{ display: "flex", height: "100%", background: "var(--bg)" }}>
            ...
            <div style={{ display: "flex", gap: 4 }}>
              {(["All", "Hot", "Warm", "Cold"] as const).map(s => (
                <button key={s} onClick={() => setScoreFilter(s)}
                  style={{
                    padding: "5px 12px", fontSize: 12, fontWeight: 600, borderRadius: 5, cursor: "pointer", transition: "all .12s",
            ...
            {(["All", ...STATUSES] as const).map(s => {
              ...
              return (
                <button key={s} onClick={() => setStatusFilter(s as LeadStatus | "All")}
                  ...
                  {s}<span style={{ fontSize: 11, opacity: 0.65 }}>({count})</span>
                </button>
              );
            })}
          </div>
            ...
            <thead style={{ position: "sticky", top: 0, zIndex: 1 }}>
              <tr style={{ background: "var(--surface-2)", borderBottom: "1px solid var(--border)" }}>
                {["Lead", "Contact", "Program", "Source", "Score", "Status", "Follow-up"].map(h => (
                  <th key={h} style={{ textAlign: "left", padding: "10px 18px", fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", color: "var(--text-muted)", whiteSpace: "nowrap" }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {filtered.map((lead, i) => {
                ...
                return (
                  <tr key={lead.id} onClick={() => handleSelectLead(lead)} className="animate-fade-up"
                    style={{ borderBottom: "1px solid var(--surface-2)", cursor: "pointer", background: isSelected ? "var(--accent-light)" : undefined, transition: "background 0.1s", animationDelay: `${Math.min(i, 10) * 25}ms` }}
                    ...
                    <td style={{ padding: "11px 18px", fontSize: 12, color: "var(--text-secondary)", fontFamily: "monospace" }}>{lead.phone}</td>
                    <td style={{ padding: "11px 18px", fontSize: 12, color: "var(--text-secondary)" }}>{lead.service}</td>
                    <td style={{ padding: "11px 18px", fontSize: 12, color: "var(--text-secondary)" }}>{lead.source}</td>
                    <td style={{ padding: "11px 18px" }}><ScoreBadge score={lead.score} /></td>
                    <td style={{ padding: "11px 18px" }}><StatusBadge status={lead.status} /></td>
                    <td style={{ padding: "11px 18px", fontSize: 12, fontFamily: "monospace", color: lead.followUpDate ? "var(--text-secondary)" : "var(--text-muted)" }}>
                      {lead.followUpDate || "—"}
                    </td>
                  </tr>
                );
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={7} style={{ padding: "52px 0", textAlign: "center", color: "var(--text-muted)", fontSize: 14 }}>
      ...
      {panelOpen && (
        <LeadDetailPanel lead={selected} onClose={handleClosePanel} onOpenFullPage={handleOpenFullPage} avatarIndex={0} />
      )}
    </div>
  );
}